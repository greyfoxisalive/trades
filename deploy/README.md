...
3. **Порты**: Убедитесь, что порты 3000, 3001, 5432, 80 и 443 не заняты другими приложениями.
...
Для включения HTTPS с Let's Encrypt сертификатами:

1. **Убедитесь, что DNS настроен**: Домен должен указывать на IP адрес вашего сервера
   ```bash
   # Проверка DNS
   dig dev.trades.anyapi.net
   ```

2. **Начальная настройка**: При первом запуске используйте `nginx.conf.initial` (HTTP только):
   ```bash
   cp deploy/nginx.conf.initial deploy/nginx.conf
   ```
   
   Или обновите домен в `nginx.conf` перед настройкой SSL.

3. **Запустите скрипт настройки SSL**:
   ```bash
   ./deploy/setup-ssl.sh [SSH_HOST] [DOMAIN] [EMAIL]
   ```
   
   Например:
   ```bash
   ./deploy/setup-ssl.sh steam-trade-droplet dev.trades.anyapi.net your-email@example.com
   ```
   
   Скрипт автоматически:
   - Создаст временную HTTP конфигурацию для получения сертификатов
   - Получит SSL сертификаты от Let's Encrypt
   - Обновит nginx.conf с полной HTTPS конфигурацией
   - Перезапустит nginx

4. **Проверьте SSL**: После выполнения скрипта ваш сайт будет доступен по HTTPS, а HTTP трафик будет автоматически перенаправляться на HTTPS.

5. **Автоматическое обновление**: Сертификаты автоматически обновляются каждые 12 часов через контейнер `certbot`.

**Важно**: 
- Порты 80 и 443 должны быть открыты в firewall
- Домен должен быть доступен из интернета перед получением сертификатов
- Первое получение сертификатов может занять несколько минут
- Если nginx не запускается из-за отсутствия сертификатов, используйте `nginx.conf.initial` до настройки SSL
